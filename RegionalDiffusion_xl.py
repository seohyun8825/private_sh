...
class RegionalDiffusionXLPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        def resize_frames(frames, split_ratio, total_width, total_height):
            """
            Resize and position frames based on a specified hierarchical split ratio.
            """
            # Split the overall height based on the first number of each row definition
            rows = split_ratio.split(';')
            row_ratios = [int(row.split(',')[0]) for row in rows]  # Row height ratios
            total_row_ratio = sum(row_ratios)
            row_heights = [total_height * ratio / total_row_ratio for ratio in row_ratios]

            resized_frames = []
            y_offset = 0

            for row, height in zip(rows, row_heights):
                columns = row.split(',')[1:]  # Skip the first number, which is for row height ratio
                column_ratios = list(map(int, columns))
                total_column_ratio = sum(column_ratios)
                x_offset = 0

                for ratio in column_ratios:
                    region_width = int(total_width * ratio / total_column_ratio)
                    if frames:
                        resized_frame = frames.pop(0).resize((region_width, int(height)), Image.Resampling.LANCZOS)
                        resized_frames.append((resized_frame, x_offset, int(y_offset)))
        ...
        def combine_frames(resized_frames, total_width, total_height):
            """
            Combine resized frames into one image based on their positions.
            """
            combined_image = Image.new('RGB', (total_width, total_height))
            for frame, x_offset, y_offset in resized_frames:
        ...
        if image_path is not None:
            ...
            # 이미지 리사이즈 및 결합
            resized_frames = resize_frames(frames, split_ratio, total_width, total_height)
            combined_image = combine_frames(resized_frames, total_width, total_height)

            # 결합된 이미지 저장
            combined_image.save(os.path.join(output_dir, "combined_image.png"))
            print(f"Combined image saved at: {os.path.join(output_dir, 'combined_image.png')}")
            ...
            # 디코딩하여 확인 (선택 사항)
            latent_decoding = user_image_latent / self.vae.config.scaling_factor
            decoded_image = self.vae.decode(latent_decoding, return_dict=False)[0]

            # 디코딩된 이미지를 저장
            decoded_image = (decoded_image.clamp(0, 1) * 255).byte()
            decoded_image_pil = Image.fromarray(decoded_image.squeeze(0).permute(1, 2, 0).cpu().numpy())
        ...
        if user_image_latent is not None:
            # **스케줄러 복사 제거**
            # scheduler_user = deepcopy(self.scheduler)  # 제거된 부분

            # 사용자 이미지 잠재 벡터를 평균 0, 표준편차 1로 정규화
            user_image_latents_as_noise = (user_image_latent - user_image_latent.mean()) / user_image_latent.std()

            # 스케줄러의 초기 노이즈 시그마 적용
            user_image_latents_as_noise *= torch.tensor(self.scheduler.init_noise_sigma).to(device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # 분류기 프리 가이던스를 사용하는 경우 잠재 벡터 확장
                latent_model_input = torch.cat([latents] * 2) if self.do_classifier_free_guidance else latents
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)

                # 디버그: Latent model input
                ...
                # 디버그: Noise Prediction
                print(f"[Step {i}] Noise pred mean: {noise_pred.mean().item()}, std: {noise_pred.std().item()}")

                # 가이던스 수행
                if self.do_classifier_free_guidance:
                    noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                    print(f"[Step {i}] Noise pred (uncond) mean: {noise_pred_uncond.mean().item()}, std: {noise_pred_uncond.std().item()}")
                    print(f"[Step {i}] Noise pred (text) mean: {noise_pred_text.mean().item()}, std: {noise_pred_text.std().item()}")

                    noise_pred = noise_pred_uncond + self.guidance_scale * (noise_pred_text - noise_pred_uncond)
                    print(f"[Step {i}] Noise pred (final) mean: {noise_pred.mean().item()}, std: {noise_pred.std().item()}")

                # 이전 노이즈 샘플 계산 x_t -> x_t-1
                latents_dtype = latents.dtype

                # 스케줄러를 사용하여 잠재 벡터 업데이트
                latents = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)[0]

                # 디버그: Updated Latents
                print(f"[Step {i}] Updated latents mean: {latents.mean().item()}, std: {latents.std().item()}")

                # 중간 결과 디코딩 및 저장 (선택 사항)
                if i % 5 == 0 or i == len(timesteps) - 1:
                    decoded_image = self.vae.decode(latents / self.vae.config.scaling_factor, return_dict=False)[0]
                    decoded_image = (decoded_image.clamp(0, 1) * 255).byte()
                    decoded_image_pil = Image.fromarray(decoded_image.squeeze(0).permute(1, 2, 0).cpu().numpy())
                    decoded_image_pil.save(f"visualize/decoded_image_step_{i}.png")
                    print(f"Intermediate decoded image saved at step {i}.")

                if latents.dtype != latents_dtype:
                    if torch.backends.mps.is_available():
                        # 일부 플랫폼에서의 dtype 불일치 처리
